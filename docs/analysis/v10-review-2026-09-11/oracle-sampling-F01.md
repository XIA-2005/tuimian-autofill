...
provenance：blue/retro 为**合成夹具**（字段族与 id 取自本仓库适配包合同的真实系统形状；liveVerified=false，D 阶段真实页核验）；wisedu-generic 为既有 jsdom 夹具。C3：三校 items 均未把"档案可能为空"的字段列入期望（见各表 optional 桶）——漏填检测由后续批次在 optional 桶扩样覆盖。
...
| yzm | text | 验证码* |  | refusable |  |
| pwd | password | 登录密码 |  | refusable |  |
...
| agree | checkbox | 本人已阅读并同意诚信承诺书* |  | refusable |  |
| yzm | text | 验证码* |  | refusable |  |
| kssj_start | text | 经历开始日期 |  | refusable |  |
| kssj_end | text | 经历结束日期 |  | refusable |  |
...
| txtAgree | checkbox | 同意页尾声明 |  | refusable |  |
| editorEssay | contenteditable | 个人陈述 |  | refusable |  |
| txtKsj | text | 科研经历起 |  | refusable |  |
| txtKss | text | 止 |  | refusable |  |
