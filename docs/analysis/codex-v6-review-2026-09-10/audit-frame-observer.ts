...
<script>
  document.addEventListener('input', function (e) {
    ...
    await lzuPage.goto('https://yjszs.lzu.edu.cn/lzuyjsytms/info');
    ...
    const countsAudit = await framePage.evaluate(() => JSON.parse(sessionStorage.getItem('tui-fill-telemetry-v1') || 'null')?.counts);
    console.log('AUDIT_FRAME_COUNTS', JSON.stringify(countsAudit));
    assert.equal(countsAudit?.total, 5, 'AUDIT: aggregate telemetry must include all five targets');
