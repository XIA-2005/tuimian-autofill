...
async function main(){
 ...
 const contract='<html><body><label>姓名<input id="xm" name="xm" value="PAGE_VALUE"></label><label>手机<input id="sjh" name="sjh"></label><label>邮箱<input id="email" name="email"></label></body></html>';
 ...
 try{
  ...
  await context.route('**/*',r=>{const u=r.request().url();if(u==='https://yjszs.lzu.edu.cn/lzuyjsytms/info')return r.fulfill({status:200,contentType:'text/html; charset=utf-8',body:contract});if(u.startsWith(base+'/')||u.startsWith('chrome-extension://'))return r.continue();external++;return r.abort()});
  ...
  await page.waitForTimeout(400);evidence.contractConflict={before:'PAGE_VALUE',after:await page.locator('#xm').inputValue(),source:'local intercepted LZU-shaped fixture'};
  await page.close();
 ...
 evidence.externalBlocked=external;writeFileSync('audit-browser-probes.json',JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2));
}
