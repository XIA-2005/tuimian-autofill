...
probe('Q11-observer-misses-silent-write',()=>{
 const d=docFor('<input name="x">');const observer=a.observer.instrument(d);const el=d.querySelector('input');el.value='WRONG';el.value='';
 return {observedHits:observer.hits.length,finalValue:el.value,expected:'detect two setter side effects'};
});
probe('Q12-e2e-button-sentinel',()=>{
 const d=new JSDOM('<script>window.__clicks={btnSubmit:0};var b=document.getElementById("btnSubmit");if(b)b.addEventListener("click",()=>window.__clicks.btnSubmit++);</script><button id="btnSubmit" type="button">submit</button>',{runScripts:'dangerously'});
 d.window.document.querySelector('button').click();return {actualClickCount:d.window.__clicks.btnSubmit,expected:1};
});
probe('Q13-background-fixed-window-and-unscoped-message',()=>{
 ...
 let handler;const timers=[],messages=[],responses=[];
 const ctx={console,crypto:require('node:crypto').webcrypto,setTimeout:(fn,ms)=>{timers.push({fn,ms});return timers.length;},clearTimeout:()=>{},chrome:{runtime:{onInstalled:{addListener:()=>{}},onMessage:{addListener:fn=>handler=fn}},tabs:{sendMessage:async(...args)=>{messages.push(args);}}}};
 vm.runInNewContext(source,ctx);
 handler({type:'PANEL_FILL'},{tab:{id:7}},r=>responses.push(r));const runId=messages[0][1].runId;
 const stats={total:1,filled:1,skipped:0,noMatch:0,profileEmpty:0,failed:0,picker:0,pickerResumeCount:0};
 handler({type:'FILL_RESULT',stats,items:[{label:'x',field:'basic.name',status:'filled',valuePreview:'SYNTHETIC_PRIVATE_VALUE'}]},{tab:{id:7},frameId:0},()=>{});
 timers[0].fn();
 handler({type:'FILL_RESULT',runId,frameSeq:2,stats:{...stats,filled:0,failed:1},items:[]},{tab:{id:7},frameId:0},()=>{});
 const done=messages.find(m=>m[1].type==='FILL_DONE')?.[1];
 return {timerMs:timers[0].ms,doneWithoutTerminal:!!done,unscopedReportCounted:done?.stats.filled===1,privatePreviewForwarded:done?.items?.[0]?.valuePreview==='SYNTHETIC_PRIVATE_VALUE',lateFailureNotApplied:done?.stats.failed===0};
});
fs.writeFileSync('audit-probes.json',JSON.stringify(findings,null,2));console.log(JSON.stringify(findings,null,2));
